...
type Dialer struct {
	...
	// establishing the transport connection.
	ProxyPacketDial func(ctx context.Context, network, address string) (net.PacketConn, error)
...
func (d *Dialer) connect(ctx context.Context, conn net.Conn, cmd Command, address string) (net.Conn, error) {
	...
	if err := d.connectAuth(conn); err != nil {
		return nil, err
	}
	...
	case ConnectCommand:
		if _, err := d.connectCommand(conn, ConnectCommand, address); err != nil {
			return nil, err
		}
		return conn, nil
	case BindCommand:
		if _, err := d.connectCommand(conn, BindCommand, address); err != nil {
			return nil, err
		}
		return conn, nil
...
func (d *Dialer) connectAuth(conn net.Conn) error {
	...
	if d.Username == "" {
		if err := writeBytes(conn, []byte{byte(noAuth)}); err != nil {
			return err
		}
	} else {
		if err := writeBytes(conn, []byte{byte(noAuth), byte(userAuth)}); err != nil {
			return err
		}
	}
	...
	case userAuth:
		...
		if _, err := conn.Write([]byte{userAuthVersion}); err != nil {
			return err
		}
		if err := writeBytes(conn, []byte(d.Username)); err != nil {
			return err
		}
		if err := writeBytes(conn, []byte(d.Password)); err != nil {
			return err
		}
